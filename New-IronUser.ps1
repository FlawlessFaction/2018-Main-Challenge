function New-MBADUser {
	...
	Process {
		...
		$DomainDC = $env:USERDNSDOMAIN.split('.')
		...
		$OU = "OU=$($Department),OU=Departments,$($DomainDC))"
		...
		foreach ($Obj in $users) {
			...
			if (!((Get-Aduser -filter {(Employeeid -like $Employeeid)} -Server $DC))) {
				...
				$UserPrincipalName = "$($UserObject.GivenName.replace(' ','').ToLower()).$($UserObject.Surname.replace(' ','').ToLower())@$($Domain)"
				...
				if ($CheckForExistingUPN -notlike $true) {
					...
					do {
						if ($UserPrincipalNamePrefix[-1] -match "[0-9]") {
							...
							#Take the existing account and increment and increment by 1
							$i++
							$UserPrincipalName = "$($UserPrincipalNamePrefix -replace "[0-9]", "$i")@$($Domain)"
						}
						else {
